...
public class WebVRCamera : MonoBehaviour
{
    ...
    private Vector3 positionVrCamera;
    private Quaternion rotationVrCamera;
    ...
    public void SetCameraPosition(string pPositionVr)
    {
        float[] array = pPositionVr.Split(',').Select(float.Parse).ToArray();
        positionVrCamera = new Vector3(array[0], array[1], array[2] * -1);
        positionVrCamera = sitStand.MultiplyPoint(positionVrCamera);
    }

    public void SetCameraRotation(string pRotationVr)
    {
        float[] array = pRotationVr.Split(',').Select(float.Parse).ToArray();
        rotationVrCamera = new Quaternion(array[0], array[1], array[2], array[3]);
    }
	...
	void changeMode(string mode)
	{
        ...
        OVRInputModule ovr_input_module = eventSystem.GetComponent<OVRInputModule>();
        StandaloneInputModule stl_input_module = eventSystem.GetComponent<StandaloneInputModule>();
        switch (mode)
		{    
		case "normal":
			...
			active = false;
            ovr_input_module.enabled = false;
            stl_input_module.enabled = true;
            break;
		case "vr":
			...
			active = true;
            ovr_input_module.enabled = true;
            stl_input_module.enabled = false;
            break;
		}
	}
    ...
    void Update()
    {
        ...
        if (active == true)
        {
            ...
            transform.rotation = new Quaternion(rotationVrCamera.x * -1, rotationVrCamera.y * -1, rotationVrCamera.z * 1, rotationVrCamera.w * 1);
            transform.position = positionVrCamera;
        }
